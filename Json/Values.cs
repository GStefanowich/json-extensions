    ...
    public static class Values {
        ...
        public static T? GetValue<T>(this JsonObject that, string key) {
            ...
            if (type == typeof(JsonObject))
                return that.TryGetPropertyValue(key, out JsonObject? @object) && @object is T val ? val : default;
            
            // If the value we're getting is a JArray
            if (type == typeof(JsonArray))
                return that.TryGetPropertyValue(key, out JsonArray? array) && array is T val ? val : default;
            
            // Fallback to JValue
            return that.TryGetPropertyValue(key, out JsonValue? value) && value.TryGetValue(out T? @out) ? @out : default;
        }
        ...
        public static IEnumerable<JsonNode?> GetValues(this JsonObject that) {
            return that.Select(pair => pair.Value);
        }
        
        /// <summary>Get all of the Values of a JsonObject</summary>
        public static IEnumerable<JsonNode?> CloneValues(this JsonObject that) {
            return that.Select(pair => pair.Value?.Clone());
        }
